...
func TestSealOpenPGP(t *testing.T) {
	data := []byte("a message to seal")

	el, err := openpgp.ReadArmoredKeyRing(bytes.NewBufferString(rsaKeys))
	if err != nil {
		t.Fatal(err)
	}

	seal, err := NewOpenPGP(el[0], data)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRoundTripOpenPGP(t *testing.T) {
	...
	want, err := NewOpenPGP(el[0], []byte("test data"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !reflect.DeepEqual(*want, *got.(*OpenPGP)) {
		t.Errorf("want OpenPGP seal %v, got %v", want, got)
	}
}
